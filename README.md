...
息屏时：A / 确定会唤醒并立刻进入对讲；B / 上 / 下只唤醒，不切频道。收到有效对讲或 VOX 触发发射也会自动亮屏。

长按 B / 上 / 下在任意状态下都可以打开菜单；本机正在发言（含 VOX）时会先结束发射。短按切频道只在空闲时生效。菜单打开时禁用对讲和 VOX。菜单内：

| 动作 | StopWatch / StickS3 | AI Passport |
| --- | --- | --- |
| 下一项 / 滚动列表 | 短按 B | 短按 上 或 下 |
| 确认 / 进入 / 保存音量或 VOX | 短按 A | 短按 确定 |
| 返回 | 长按 B | 长按 上 或 下 |
