...
// esp32c3-walkie 移植补丁:esp_lcd_panel_draw_bitmap 的颜色传输是异步 DMA。
// LVGL 单缓冲场景下,flush 返回前必须等 DMA 读完用户缓冲,否则下一帧渲染
// 会撕裂上一帧。上游 ai-passport 用 esp_lvgl_port 处理,这里改用信号量。
static SemaphoreHandle_t         s_color_done;

static bool on_color_trans_done(esp_lcd_panel_io_handle_t io, esp_lcd_panel_io_event_data_t *edata, void *ctx) {
    (void)io; (void)edata; (void)ctx;
    BaseType_t hp_task = pdFALSE;
    if (s_color_done) xSemaphoreGiveFromISR(s_color_done, &hp_task);
...
esp_err_t bsp_display_init(void) {
    ...
    esp_lcd_panel_io_spi_config_t io_cfg = {
        ...
        // LVGL 单缓冲每次 flush 只发一块 bitmap，队列深度 1 保证一次 Give 对应一次 wait。
        .trans_queue_depth = 1,
...
bool bsp_display_wait_color_trans_done(uint32_t timeout_ms) {
    if (!s_color_done) return true;
    return xSemaphoreTake(s_color_done, pdMS_TO_TICKS(timeout_ms)) == pdTRUE;
}

esp_err_t bsp_display_clear(uint16_t rgb565) {
    if (s_panel == NULL) return ESP_ERR_INVALID_STATE;
    enum { kRows = 16 };
    const size_t pixels = (size_t)BSP_LCD_W * kRows;
    uint16_t *buf = heap_caps_malloc(pixels * sizeof(uint16_t), MALLOC_CAP_DMA);
    if (buf == NULL) return ESP_ERR_NO_MEM;
    for (size_t i = 0; i < pixels; ++i) buf[i] = rgb565;
    esp_err_t result = ESP_OK;
    for (int y = 0; y < BSP_LCD_H; y += kRows) {
        const int height = (y + kRows <= BSP_LCD_H) ? kRows : (BSP_LCD_H - y);
        (void)bsp_display_wait_color_trans_done(0);
        if (esp_lcd_panel_draw_bitmap(s_panel, 0, y, BSP_LCD_W, y + height, buf) != ESP_OK) {
            result = ESP_FAIL;
            break;
        }
        if (!bsp_display_wait_color_trans_done(100)) {
