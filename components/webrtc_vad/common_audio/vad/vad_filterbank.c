...
static void HighPassFilter(const int16_t* data_in,
    ...
    for (size_t i = 0; i < data_length; i++) {
        int32_t tmp32 = kHpZeroCoefs[0] * *in_ptr;
...
static void AllPassFilter(const int16_t* data_in,
    ...
    for (size_t i = 0; i < data_length; i++) {
        const int32_t tmp32 = state32 + filter_coefficient * *data_in;
        const int16_t tmp16 = (int16_t)(tmp32 >> 16);
        *data_out++ = tmp16;
...
static void SplitFilter(const int16_t* data_in,
                        ...
                        int16_t* hp_data_out,
                        int16_t* lp_data_out) {
    const size_t half_length = data_length >> 1;
    ...
    for (size_t i = 0; i < half_length; i++) {
        const int16_t tmp_out = *hp_data_out;
...
static void LogOfEnergy(const int16_t* data_in,
                        ...
                        int16_t* log_energy) {
    int tot_rshifts = 0;
    uint32_t energy = (uint32_t)WebRtcSpl_Energy((int16_t*)data_in, data_length, &tot_rshifts);

    if (energy != 0) {
        const int normalizing_rshifts = 17 - WebRtcSpl_NormU32(energy);
        ...
        log2_energy = (int16_t)(log2_energy + (int16_t)((energy & 0x00003FFF) >> 4));
        *log_energy = (int16_t)(((kLogConst * log2_energy) >> 19) + ((tot_rshifts * kLogConst) >> 9));
        if (*log_energy < 0) *log_energy = 0;
    } else {
        *log_energy = offset;
        return;
    }

    *log_energy = (int16_t)(*log_energy + offset);

    if (*total_energy <= kMinEnergy) {
        if (tot_rshifts >= 0) {
            *total_energy = (int16_t)(*total_energy + kMinEnergy + 1);
        } else {
            *total_energy = (int16_t)(*total_energy + (int16_t)(energy >> -tot_rshifts));
        }
    }
}
